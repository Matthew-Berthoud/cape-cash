...
const generatePdfDoc = async (expenseItems: ExpenseItem[], receipts: Receipt[]) => {
  const { jsPDF } = window.jspdf;
  const doc = new jsPDF();
  
  const tableColumn = ["Date", "Category", "Customer/Project", "Description", "Amount"];
  ...
  const allReceiptIds = new Set(expenseItems.flatMap(item => item.receiptImageIds));
  ...
  for (const receipt of uniqueReceipts) {
    ...
    try {
        ...
        console.error("Could not add image to PDF, it might be corrupted.", error);
        doc.addPage();
...
const PdfPreviewPage: React.FC<PdfPreviewPageProps> = ({ expenseItems, receipts, onBack }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto bg-white dark:bg-slate-800 p-8 rounded-lg shadow-lg">
      <h2 className="text-xl font-semibold mb-6 text-slate-700 dark:text-slate-300">Step 3: Preview & Download</h2>

      <div className="bg-slate-100 dark:bg-slate-900 rounded-md p-4 min-h-[70vh]">
        {isLoading ? (
          <div className="flex justify-center items-center h-full">
            <p className="text-slate-500 dark:text-slate-400">Generating PDF...</p>
          </div>
        ) : pdfUrl ? (
          <iframe src={pdfUrl} title="Expense Report" className="w-full h-[70vh] border-0 rounded-md"></iframe>
        ) : (
          <div className="flex justify-center items-center h-full">
            <p className="text-slate-500 dark:text-slate-400">No expenses to display.</p>
          </div>
        )}
      </div>
