  ...
  return (
    <div className="max-w-4xl mx-auto bg-white dark:bg-slate-800 p-8 rounded-lg shadow-lg">
      ...
      <p className="mb-6 text-slate-500 dark:text-slate-400">
        Select one or more receipt images. The app will use AI to automatically
        extract the details.
      </p>
        ...
        <div className="mt-8">
          ...
          <div className="space-y-4">
            {uploadStatuses.map((item, index) => (
              <div
                key={index}
                className="flex items-center gap-4 p-3 bg-slate-50 dark:bg-slate-700 rounded-md"
              >
                {item.status === "success" && item.receipt ? (
                  <img
                    src={`data:image/jpeg;base64,${item.receipt.base64}`}
                    alt={item.fileName}
