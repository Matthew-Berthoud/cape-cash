...
export async function parseReceipt(base64Image: string): Promise<{ date: string; description: string; amount: number; category: string; }> {
    const imagePart = {
        inlineData: {
            mimeType: 'image/jpeg',
            data: base64Image,
    ...
    const textPart = {
        text: `Analyze the provided receipt image. Extract the transaction date, a short description of the vendor, the total amount, and select the best category from the list. The default category should be '8190 G&A Office supplies' if unsure.`
    };
    ...
    try {
        ...
        throw new Error("Failed to parse receipt. Please check your API key and network connection.");
    }
}