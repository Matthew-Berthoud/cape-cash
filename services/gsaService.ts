...
export async function fetchPerDiemRates(startDate: string, location: Trip['location']): Promise<PerDiemRates> {
    ...
    try {
        ...
        if (error instanceof Error && (error.message.includes('404') || error.message.includes('No per diem rates'))) {
             throw new Error("Could not find rates for this location. Please check spelling or try a ZIP code.");
        }
        throw error;
    }
}
